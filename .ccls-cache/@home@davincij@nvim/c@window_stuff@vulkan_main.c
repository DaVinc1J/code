#include "vulkan_fonts.h"
#include "vulkan_global.h"
#include "vulkan_ui.h"

SDL_Texture *update_texture(graphics *gfx, TTF_Font *font, char *text,
                            SDL_Colour *text_colour,
                            SDL_Colour *background_colour) {
  SDL_Surface *surface =
      TTF_RenderUTF8_Shaded(font, text, *text_colour, *background_colour);
...
void render_rects(graphics *gfx, generic_rect *rect, global_values *global) {
  ...
  if (check_state(global->needs_update, rect->common->id)) {
    ...
    case INPUT:
      if (strlen(input->input_text) == 0 && input->is_previewed) {
        rect->common->textures.text_texture =
            update_texture(gfx,
                           gfx->fonts[(input->preview_font >> 2) & 0x03]
                                     [input->preview_font & 0x03],
                           input->preview_text, &input->preview_colour,
                           &rect->common->colours.main_obj);

      } else {
        rect->common->textures.text_texture = update_texture(
            gfx,
            gfx->fonts[(rect->common->font >> 2) & 0x03]
                      [rect->common->font & 0x03],
            input->input_text, &rect->common->colours.content_text,
            &rect->common->colours.main_obj);
      }
      break;
    case TEXT:
      rect->common->textures.text_texture =
          update_texture(gfx,
                         gfx->fonts[(rect->common->font >> 2) & 0x03]
                                   [rect->common->font & 0x03],
                         text->label, &rect->common->colours.content_text,
                         &rect->common->colours.main_obj);
      ...
      if (button->render_click) {
        rect->common->textures.text_texture =
            update_texture(gfx,
                           gfx->fonts[(rect->common->font >> 2) & 0x03]
                                     [rect->common->font & 0x03],
                           button->label, &button->on_click->content_text,
        ...
        rect->common->textures.text_texture =
            update_texture(gfx,
                           gfx->fonts[(rect->common->font >> 2) & 0x03]
                                     [rect->common->font & 0x03],
                           button->label, &rect->common->colours.content_text,
                           &rect->common->colours.main_obj);
      }
    ...
    if (rect->common->textures.text_texture) {
      ...
      case INPUT:
        if (input->is_previewed && strlen(input->input_text) == 0) {
          TTF_SizeText(gfx->fonts[(input->preview_font >> 2) & 0x03]
                                 [input->preview_font & 0x03],
                       input->preview_text, &text_width, &text_height);
        } else {
          TTF_SizeText(gfx->fonts[(rect->common->font >> 2) & 0x03]
                                 [rect->common->font & 0x03],
                       input->input_text, &text_width, &text_height);
        }
        break;
      case TEXT:
        TTF_SizeText(gfx->fonts[(rect->common->font >> 2) & 0x03]
                               [rect->common->font & 0x03],
                     text->label, &text_width, &text_height);
        break;
      case BUTTON:
        TTF_SizeText(gfx->fonts[(rect->common->font >> 2) & 0x03]
                               [rect->common->font & 0x03],
                     button->label, &text_width, &text_height);
        break;
      default:
        break;
      }
      ...
      if (check_state(global->is_centred, rect->common->id)) {
        rect->common->textures.text_rect.x =
            rect->common->boxes.main_box.x +
            (rect->common->boxes.main_box.w - text_width) / 2;
      } else {
        rect->common->textures.text_rect.x = rect->common->boxes.main_box.x + 5;
      }

      rect->common->textures.text_rect.y =
          rect->common->boxes.main_box.y +
  ...
  switch (rect->type) {
  case INPUT:
    SDL_SetRenderDrawColor(gfx->ren, rect->common->colours.border_obj.r,
    ...
    SDL_SetRenderDrawColor(gfx->ren, rect->common->colours.main_obj.r,
  ...
  case BUTTON:
    if (button->render_click) {
      SDL_SetRenderDrawColor(gfx->ren, button->on_click->border_obj.r,
                             button->on_click->border_obj.g,
                             button->on_click->border_obj.b,
                             button->on_click->border_obj.a);
      SDL_RenderFillRect(gfx->ren, &rect->common->boxes.border_box);
      SDL_SetRenderDrawColor(
          gfx->ren, button->on_click->main_obj.r, button->on_click->main_obj.g,
          button->on_click->main_obj.b, button->on_click->main_obj.a);
      SDL_RenderFillRect(gfx->ren, &rect->common->boxes.main_box);
    } else {
      SDL_SetRenderDrawColor(gfx->ren, rect->common->colours.border_obj.r,
                             rect->common->colours.border_obj.g,
  ...
  SDL_RenderCopy(gfx->ren, rect->common->textures.text_texture, NULL,
                 &rect->common->textures.text_rect);
  ...
  case INPUT:
    if (global->render_cursor && input->is_active) {
      int text_width =
          font_text_width(gfx->fonts[(rect->common->font >> 2) & 0x03]
                                    [rect->common->font & 0x03],
                          input->input_text, global->cursor_pos);
      ...
      cursor_y = rect->common->textures.text_rect.y +
                 rect->common->textures.text_rect.h;

      SDL_SetRenderDrawColor(gfx->ren, 0, 0, 0, 255);
      SDL_RenderDrawLine(gfx->ren, cursor_x, cursor_y, cursor_x + cursor_width,
                         cursor_y);
    }
    break;
  case TEXT:
...
void render_objects(graphics *gfx, global_values *global, generic_rect **rects,
                    int *rect_count) {
...
void check_events(graphics *gfx, keymap *keys, global_values *global,
                  generic_rect **rects, int *rect_count) {
  while (SDL_PollEvent(&gfx->event)) {
    ...
    case SDL_KEYDOWN:
      ...
      if (global->active_input_rect) {
        input_specific *data =
            (input_specific *)global->active_input_rect->data;
        switch (gfx->event.key.keysym.sym) {
        case SDLK_BACKSPACE:
          if (global->cursor_pos > 0) {
            memmove(&data->input_text[global->cursor_pos - 1],
                    &data->input_text[global->cursor_pos],
                    strlen(data->input_text) - global->cursor_pos + 1);
            global->cursor_pos--;
            set_state(global->needs_update,
                      global->active_input_rect->common->id);
          }
          break;
    ...
    case SDL_TEXTINPUT:
      if (global->active_input_rect) {
        ...
        memmove(&data->input_text[global->cursor_pos +
                                  strlen(gfx->event.text.text)],
                &data->input_text[global->cursor_pos],
                strlen(data->input_text) - global->cursor_pos + 1);

        memcpy(&data->input_text[global->cursor_pos], gfx->event.text.text,
               strlen(gfx->event.text.text));
    ...
    case SDL_MOUSEBUTTONDOWN:
      ...
      for (int i = 0; i < *rect_count; i++) {
        ...
        if (SDL_PointInRect(&(SDL_Point){x, y},
                            &rects[i]->common->boxes.main_box)) {
          switch (rects[i]->type) {
          case INPUT:
            if (global->active_input_rect &&
                global->active_input_rect != rects[i]) {
              input_specific *prev_input =
                  (input_specific *)global->active_input_rect->data;
            ...
            if (!button->render_click) {
              ...
              for (int j = 0; j < rects[i]->common->children_count; j++) {
                ...
                case INPUT:
                  ...
                  printf("%s: %s\n", child_input->preview_text,
                         child_input->input_text);
                  memset(child_input->input_text, 0,
                         strlen(child_input->input_text) + 1);
                  global->cursor_pos = 0;
        ...
        if (global->active_input_rect) {
          if (!SDL_PointInRect(
                  &(SDL_Point){x, y},
                  &global->active_input_rect->common->boxes.main_box)) {
...
int main() {
  ...
  int rect_count = 3;
  generic_rect **rects =
      (generic_rect **)malloc(sizeof(generic_rect *) * rect_count);
  ...
  *(rects[0]->common) = (rect_common){
      .boxes = {.main_box = (SDL_Rect){66, 100, 434, 30},
  ...
  *(rects[2]->common) = (rect_common){
      .boxes = {.main_box = (SDL_Rect){66, 150, 434, 30},
  ...
  *(rects[1]->common) = (rect_common){
      .boxes = {.main_box = (SDL_Rect){66, 200, 60, 30},
  ...
  rects[1]->common->children =
      malloc(sizeof(generic_rect *) * rects[1]->common->children_count);
