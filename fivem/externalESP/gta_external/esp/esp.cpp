...
D3DXVECTOR2 c_esp::world_to_screen(D3DXVECTOR3 world_pos) {
  static uintptr_t viewport = c_mem::get()->read_mem<uintptr_t>(
      g::base_address.modBaseAddr +
      (g::is_steam_version ? (0x0208BAF0) : (0x02087780)));
  auto viewmatrix = c_mem().read_mem<D3DXMATRIX>(viewport + 0x24C);

  D3DXMatrixTranspose(&viewmatrix, &viewmatrix);
  auto vec_x = D3DXVECTOR4(viewmatrix._21, viewmatrix._22, viewmatrix._23,
                           viewmatrix._24),
       vec_y = D3DXVECTOR4(viewmatrix._31, viewmatrix._32, viewmatrix._33,
                           viewmatrix._34),
       vec_z = D3DXVECTOR4(viewmatrix._41, viewmatrix._42, viewmatrix._43,
                           viewmatrix._44);

  auto screen_pos =
      D3DXVECTOR3((vec_x.x * world_pos.x) + (vec_x.y * world_pos.y) +
                      (vec_x.z * world_pos.z) + vec_x.w,
                  (vec_y.x * world_pos.x) + (vec_y.y * world_pos.y) +
                      (vec_y.z * world_pos.z) + vec_y.w,
                  (vec_z.x * world_pos.x) + (vec_z.y * world_pos.y) +
                      (vec_z.z * world_pos.z) + vec_z.w);
...
bool c_esp::get_bounding_box(D3DXVECTOR3 origin, D3DXVECTOR3 mins,
                             D3DXVECTOR3 maxs, int &out_x, int &out_y,
                             int &out_w, int &out_h) {
  D3DXVECTOR3 min = (origin + mins);
  D3DXVECTOR3 max = (origin + maxs);
  D3DXVECTOR3 points[] = {
      D3DXVECTOR3(min.x, min.y, min.z), D3DXVECTOR3(min.x, max.y, min.z),
      D3DXVECTOR3(max.x, max.y, min.z), D3DXVECTOR3(max.x, min.y, min.z),
      D3DXVECTOR3(max.x, max.y, max.z), D3DXVECTOR3(min.x, max.y, max.z),
      D3DXVECTOR3(min.x, min.y, max.z), D3DXVECTOR3(max.x, min.y, max.z)};
...
void c_esp::draw_skeleton(uintptr_t ped_base) {
  int bone_positions[][2] = {{0, 7}, {7, 6}, {7, 5}, {7, 8}, {8, 3}, {8, 4}};
  for (int i = 0; i < 6; ++i) {
    ...
    if (bone_1 != D3DXVECTOR3(0, 0, 0) && bone_2 != D3DXVECTOR3(0, 0, 0) &&
        ((screen_1 = world_to_screen(bone_1)) != D3DXVECTOR2(0, 0)) &&
        ((screen_2 = world_to_screen(bone_2)) != D3DXVECTOR2(0, 0))) {
      rendering::c_renderer::get()->draw_line(screen_1.x, screen_1.y,
                                              screen_2.x, screen_2.y,
...
void c_esp::draw_esp() {
  static auto replay_interface =
      sdk::c_replay_interface(c_mem::get()->read_mem<uintptr_t>(
          g::base_address.modBaseAddr +
          (g::is_steam_version ? (0x01F00028) : (0x01EFD4C8))));
  ...
  if (!local_ped.base) {
    local_ped =
        sdk::c_ped(c_mem::get()->read_mem<uintptr_t>(g::world_ptr + 0x8));
  ...
  auto local_origin =
      c_mem::get()->read_mem<D3DXVECTOR3>(local_ped.get_navigation() + 0x50);
  if (local_ped.get_playerinfo_ptr()) {
    auto playerinfo = local_ped.get_playerinfo();
    auto local = local_ped.get_ped();
    auto cur_vehicle =
        c_mem::get()->read_mem<uint64_t>(local_ped.base + 0x0D28);
    ...
    if (vars::player::teleport_to_waypoint || vars::esp::draw_waypoint) {
      ...
      for (int i = 0; i < 2000; i++) {
        ...
        if ((blip_color != /*BlipColor::ColorWaypoint*/ 84) ||
            (blip_icon != /*BlipSprite::SpriteWaypoint*/ 8))
          continue;

        auto waypoint_pos = c_mem::get()->read_mem<D3DXVECTOR3>(blip + 0x10);
        if (waypoint_pos != D3DXVECTOR2(0.f, 0.f)) {
          auto waypoint_up = world_to_screen(D3DXVECTOR3(
              waypoint_pos.x, waypoint_pos.y, waypoint_pos.z + 220.f));
          auto screen_waypoint_pos = world_to_screen(
              D3DXVECTOR3(waypoint_pos.x, waypoint_pos.y, waypoint_pos.z));
          if (waypoint_up != D3DXVECTOR2(0, 0) &&
              screen_waypoint_pos != D3DXVECTOR2(0, 0)) {
            rendering::c_renderer::get()->draw_line(
                screen_waypoint_pos.x, screen_waypoint_pos.y, waypoint_up.x,
                waypoint_up.y, D3DCOLOR_RGBA(255, 0, 0, 255));
            rendering::c_renderer::get()->draw_string(
                waypoint_up.x, waypoint_up.y - 14.f, d3d9::tahoma_13,
                D3DCOLOR_RGBA(255, 0, 0, 255), DT_CENTER, false,
                (L"WAYPOINT - " +
                 std::to_wstring(
                     int(D3DXVec3Length(&(local_origin - waypoint_pos)))) +
                 L"m")
                    .c_str());
          }
          ...
          if (vars::player::teleport_to_waypoint) {
            for (int i = 0; i < 5; i++) {
              if (waypoint_pos != D3DXVECTOR2(0.f, 0.f)) {
                auto vehicle_navigation = c_mem::get()->read_mem<uintptr_t>(
                    local_ped.get_vehicle() + 0x0030);
                c_mem::get()->write_mem<D3DXVECTOR3>(
                    vehicle_navigation + 0x50,
                    D3DXVECTOR3(waypoint_pos.x, waypoint_pos.y,
                                teleport_height));
                c_mem::get()->write_mem<D3DXVECTOR3>(
                    local_ped.get_navigation() + 0x50,
                    D3DXVECTOR3(waypoint_pos.x, waypoint_pos.y,
                                teleport_height));
              }
            }
  ...
  menu_framework->add_entry(L"PLAYERLIST", &draw_plist,
                            c_menu_framework::tab_type::type_tab);
  ...
  for (auto i = 0U; i < pedlist_interface.max_ptrs; i++) {
    auto ped =
        sdk::c_ped(c_mem::get()->read_mem<uintptr_t>(ped_list + (i * 0x10)));
    if (!ped.base)
      continue;
    ...
    if (ped.get_playerinfo_ptr()) {
      if (draw_plist) {
        ...
        if (ped.base != local_ped.base)
          menu_framework->add_entry(
              (L"  " + std::to_wstring(i) + L", " +
               std::wstring(player_name.begin(), player_name.end()))
                  .c_str(),
              &player_var[i], c_menu_framework::tab_type::type_dont_draw);
        if (player_var[i]) {
          menu_framework->add_entry(L"Teleport to", &teleport_to_player[i],
                                    c_menu_framework::tab_type::type_on_off);
        }
      }
      if (teleport_to_player[i]) { // this shit buggy af, needs to be looped
                                   // couple times in order to teleport
                                   // anywhere. might also teleport u in random
                                   // locations lol.
        for (int i = 0; i < 5; i++) {
          auto vehicle_navigation = c_mem::get()->read_mem<uintptr_t>(
              local_ped.get_vehicle() + 0x0030);
          c_mem::get()->write_mem<D3DXVECTOR3>(vehicle_navigation + 0x50,
                                               ped_origin);
          c_mem::get()->write_mem<D3DXVECTOR3>(
              local_ped.get_navigation() + 0x50, ped_origin);
        }
    ...
    auto mins = D3DXVECTOR3(-0.61f, -0.25f, -1.30f),
         maxs = D3DXVECTOR3(0.61f, 0.25f, 0.94f);
    ...
    if (get_bounding_box(ped_origin, mins, maxs, x, y, w, h)) {
      if (vars::esp::draw_box) {
        auto box_col = (ped_type == sdk::NETWORK_PLAYER)
                           ? D3DCOLOR_RGBA(255, 0, 0, 255)
                           : D3DCOLOR_RGBA(0, 255, 0, 255);
        rendering::c_renderer::get()->draw_rect(x, y, w, h,
                                                D3DCOLOR_RGBA(0, 0, 0, 255));
        rendering::c_renderer::get()->draw_rect(x + 1, y + 1, w - 2, h - 2,
                                                box_col);
        rendering::c_renderer::get()->draw_rect(x + 2, y + 2, w - 4, h - 4,
                                                D3DCOLOR_RGBA(0, 0, 0, 255));
      }
      if (vars::esp::draw_name)
        rendering::c_renderer::get()->draw_string(
            x + (w / 2), y - 13, d3d9::tahoma_13,
            D3DCOLOR_RGBA(255, 255, 255, 255), DT_CENTER, false,
      ...
      if (vars::esp::draw_weapon) {
        ...
        if (weapon_mgr) {
          auto weapon_current =
              c_mem::get()->read_mem<uintptr_t>(weapon_mgr + 0x20);
          if (weapon_current) {
            auto weapon_hash =
                c_mem::get()->read_mem<uint32_t>(weapon_current + 0x10);
            rendering::c_renderer::get()->draw_string(
                x + (w / 2), y + h + 2, d3d9::tahoma_13,
                D3DCOLOR_RGBA(255, 255, 255, 255), DT_CENTER, false,
                sdk::get_weapon_name(weapon_hash).c_str());
          }
        }
      }
    }
...
void c_esp::draw_object_esp(sdk::c_replay_interface replay_iface) {
  ...
  if (vars::esp::draw_custom_hash && !load_custom_hash) {
    ...
    GetPrivateProfileStringA("DRUG_ESP", "custom_hash_to_draw", 0, value_output,
                             32, filepath.c_str());
    custom_hash = std::stoull(value_output);
    GetPrivateProfileStringA("DRUG_ESP", "custom_hash_name", 0, value_output,
                             32, filepath.c_str());
  ...
  for (auto i = 0U; i < objectlist_interface.max_objects; i++) {
    ...
    if (position != D3DXVECTOR3(0, 0, 0)) {
      ...
      if (vars::esp::draw_custom_hash && custom_hash != 0 &&
          weapon_hash == custom_hash)
        namee = std::wstring(custom_hash_name.begin(), custom_hash_name.end());

      if (!namee.empty())
        rendering::c_renderer::get()->draw_string(
            w2s.x, w2s.y, d3d9::tahoma_13, D3DCOLOR_RGBA(255, 0, 0, 255),
            DT_CENTER, false, namee.c_str());
    }
  }
}
